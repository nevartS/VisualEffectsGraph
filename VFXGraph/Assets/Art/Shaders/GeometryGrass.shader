...
Shader "HiGames/GeometryGrass"
{
    Properties
    {
        ...
        _MinDistance("Min Distance", Float) = 40
    ...
    struct Attributes
    {
        // The position_OS variable contains the vertex positions in object space
    ...
    uniform float3 _DeltaPosition;

    v2g vert(Attributes v)
    {
        float3 v0 = v.position_OS.xyz;

        v2g OUT;
        OUT.pos = v.position_OS;
        OUT.norm = v.normal;
        OUT.uv = v.texcoord;
        OUT.color = v.color;
    ...
    void geom(point v2g IN[1], inout TriangleStream<g2f> triStream)
    {
        ...
        // Just use an up facing normal, works nice
        ...
        float distanceFade = 1 - saturate((distanceFromCamera - _MinDistance) / _MaxDistance);

        // Wind
        float3 v0 = IN[0].pos.xyz;
        float3 wind1 = float3(sin(_Time.x * _WindSpeed + v0.x) +
                              sin(_Time.x * _WindSpeed + v0.z * 2) +
                              sin(_Time.x * _WindSpeed * 0.1 + v0.x), 0,
                              cos(_Time.x * _WindSpeed + v0.x * 2) +
                              cos(_Time.x * _WindSpeed + v0.z));

        wind1 *= _WindStrength;

        // Interactivity
        float3 dis = distance(_DeltaPosition, worldPos); // Distance for radius
        float3 radius = 1 - saturate(dis / _Radius); // In world radius based on object interaction radius
        float3 sphereDisp = worldPos - _DeltaPosition; // Position comparison
        sphereDisp *= radius; // Position multiplied by radius for falloff / increase strength
        sphereDisp = clamp(sphereDisp.xyz * _Strength, -0.8, 0.8);

        // Set vertex color
        float3 color = (IN[0].color).rgb;

        // Set grass height from tool, uncomment if you're not using the tool
        ...
        for (int j = 0; j < (GrassBlades * distanceFade); j++)
        {
            ...
            triStream.Append(GrassVertex(v0 + float3(sphereDisp.x * 1.5, sphereDisp.y, sphereDisp.z * 1.5) +
                                         wind1, 0, _GrassHeight, offset, forward, float2(0.5, 1), transformationMatrix,
                                         faceNormal, color));

            // Restart the strip oto start another grass blade
